...
|--------|------|--------|-------------|
| `frankenphp_threads_total` | gauge | `state` (`busy`, `idle`, `other`) | Number of threads by state. Despite the `_total` suffix (normally reserved for counters), this is a gauge; the name is kept for backward compatibility. |
